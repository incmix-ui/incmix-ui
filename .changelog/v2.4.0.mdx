---
title: Version 2.4.0
description:
  Explore the changelog for Incmix UI version 2.4.0. Learn about the latest
  features, bug fixes, and improvements.
releaseUrl: https://github.com/incmix-ui/incmix-ui/pull/6948
releaseDate: November 14, 2022
version: 2.4.0
---

## All packages

Fix issue where using `@incmix-ui/react` or any sub-package in a TypeScript
project with `"type": "module"` in `package.json` and
`"moduleResolution": "Node16"` in `tsconfig.json` cannot find the types.

## @incmix-ui/card@2.1.0

Introduced new `Card` component

```jsx live=false
export function Basic() {
  return (
    <Card>
      <CardHeader>
        <Heading size="md"> Customer dashboard</Heading>
      </CardHeader>
      <CardBody>
        <Text>View a summary of all your customers over the last month.</Text>
      </CardBody>
      <CardFooter>
        <Button>View here</Button>
      </CardFooter>
    </Card>
  )
}
```

## @incmix-ui/system@2.3.2

Remove `Dict` type dependency from `createStyleContext`

## @incmix-ui/tooltip@2.2.1

Fixed issue where dynamically changing a tooltip's `isDisabled` when it is open
didn't have any effect.

Now, changing the `isDisabled` prop will close the tooltip leading to a more
consistent behavior.

## @incmix-ui/theme@2.2.0

Refactor theme to use CSS variables
