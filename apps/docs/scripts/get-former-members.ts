...
async function main() {
  fs.writeFileSync(CONFIG_PATH, JSON.stringify(await generateFormerMemberData(), null, 2))
}
...
async function generateFormerMemberData() {
  ...
  const tableLines = (data as any).split('\n').filter(line => line.startsWith('|'))
...
const parseRepoData = (lines: string[]) => {
  ...
  for (const line of lines) {
    ...
    const components = segments[1].split(',').map(component => normalizeString(component) || null)

    parsedData.push({
      name: normalizeString(segments[0].slice(0, segments[0].indexOf('@'))) || null,
      githubName: normalizeString(segments[0].slice(segments[0].indexOf('@') + 1)) || null,
