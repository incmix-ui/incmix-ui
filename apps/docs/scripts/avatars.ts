...
async function getSponsors() {
  const response = await fetch('https://opencollective.com/incmix-ui/members/all.json')
  ...
  const sponsors = unfilteredSponsors.filter(
    (currentSponsor, index, allSponsors) => index === allSponsors.findIndex(s => s.profile === currentSponsor.profile),
  )

  const individuals = sponsors.filter(sponsor => sponsor.type === 'USER' && sponsor.image != null)
...
async function buildSponsors() {
  ...
  const individualSponsors = await Promise.all(
    individuals.map(async individual => {
      const filename = await individualAvatarsCache.urlToFile(individual.image, individual.MemberId.toString())
      return {
        ...individual,
  ...
  const companySponsors = await Promise.all(
    companies.map(async company => {
      const filename = await companyAvatarsCache.urlToFile(company.image, company.MemberId.toString())
      return {
        ...company,
        image: `/avatars/${filename}`,
      }
