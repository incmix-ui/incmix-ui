import { fileToPath, parseMarkdownFile, posixPath, removePrefix } from '@docusaurus/utils'
import fs from 'fs'
...
async function getMDXMeta(file: string) {
  ...
  const { content, frontMatter: _frontMatter } = await parseMarkdownFile(filePath)
  ...
  const slug = fileToPath(filePath).replace(`/${websiteRoot}`, '').replace(processCWD, '')
...
async function getSearchMeta() {
  ...
  for (const file of files) {
    ...
    const isExcluded = !!excludedSlugs.find(excludedSlug => fileToPosixPath.includes(excludedSlug))
