import { Badge, Box, incmix, Flex } from '@incmix-ui/react'
import { SkipNavContent, SkipNavLink } from '@incmix-ui/skip-nav'
...
function PageContainer(props: PageContainerProps) {
  const { frontmatter, children, leftSidebar, rightSidebar, pagination, hideToc, maxWidth = '48rem' } = props
  ...
  return (
    <>
      <SkipNavLink zIndex={20}>{t('component.page-container.skip-to-content')}</SkipNavLink>
      <Header />
            ...
            <Box id="content" px={5} mx="auto" minH="76vh">
              <Flex>
                <Box minW="0" flex="auto" px={{ base: '4', sm: '6', xl: '8' }} pt="10">
                  <Box maxW={maxWidth}>
                    <incmix.h1 tabIndex={-1} outline={0} apply="mdx.h1">
                ...
                {!hideToc && (
                  <TableOfContent visibility={headings.length === 0 ? 'hidden' : 'initial'} headings={headings} />
                )}
                {rightSidebar}
              </Flex>
            </Box>
