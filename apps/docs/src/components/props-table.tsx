import * as ComponentProps from '@incmix-ui/props-docs'
import { incmix, Code, Flex, HStack, Stack, theme } from '@incmix-ui/react'
import { isObject, isString } from '@incmix-ui/utils'
  ...
  return (
    <Stack overflowX="auto" spacing="16" my="10">
      {propList.map(prop => (
        ...
        >
          <incmix.div css={{ textAlign: 'start', fontSize: '1em' }}>
            <incmix.h3
              css={{
                fontSize: '0.8em',
                paddingBottom: 4,
            ...
            >
              <HStack>
                ...
                {prop.required && <Code colorScheme="red">{t('component.props-table.required')}</Code>}
              </HStack>
            </incmix.h3>
          </incmix.div>
          <div>
            {prop.description && (
              <Flex>
                <div className="row">{t('component.props-table.description')}</div>
                <div className="cell">
...
const TYPE_GENERIC_THEMEABLE = ['string', 'string | (string & {})', '(string & {})']

const isGenericThemeable = (type: string) => TYPE_GENERIC_THEMEABLE.includes(type)
...
function isColorScheme(value: unknown): value is Record<string, string> {
  return isObject(value) && ['50', '100', '200', '300', '400', '600', '700', '800', '900'].every(k => isString(value[k]))
}
...
function makePropsTable({ of, omit, only }: MakePropsTableOptions) {
  ...
  const featNotImplemented = (feat: string) => (
    <>
      ...
      {t('component.props-table.are-not-implemented-in-the-default-theme')} {t('component.props-table.you-can')}{' '}
      <Link href="/docs/theming/customize-theme#customizing-component-styles" passHref>
