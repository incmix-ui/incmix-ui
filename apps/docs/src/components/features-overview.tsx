import { Flex, Heading, Icon, SimpleGrid, Stack, Text } from '@incmix-ui/react'
...
const Feature = ({ title, icon, children, ...props }) => {
  return (
    <Stack
      ...
      {...props}
    >
      <Flex rounded="12px" w="12" h="12" bg="teal.500" align="center" justify="center">
...
export const FeaturesOverview = () => {
  ...
  const features = featureSidebar[asPath].routes?.[0].routes.filter(feature => feature.path !== asPath)
  ...
  return (
    <SimpleGrid mt="12" minChildWidth="15.625rem" spacing="8">
      {features.map(feature => (
        <Link key={feature.title} passHref href={`${feature.routes?.[0].path}?scroll=true`}>
          <Feature icon={icons[feature.title] ?? null} title={feature.title}>
            {feature.summarize ? `${feature.routes.length} ${changeFeatureText(feature.path)}` : null}
          </Feature>
        </Link>
      ))}
    </SimpleGrid>
