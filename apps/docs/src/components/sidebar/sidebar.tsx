/* eslint-disable */
// @ts-nocheck
import { Badge, Box, Center, incmix, HStack, List, ListItem, ListProps } from '@incmix-ui/react'
...
export function SidebarContent({ routes, pathname, contentRef }: SidebarContentProps) {
  return (
    <>
      {routes.map((lvl1, idx) => {
        return (
          <Fragment key={idx}>
            {lvl1.heading && (
              <incmix.h4 fontSize="sm" fontWeight="bold" my="4" textTransform="uppercase" letterSpacing="wider">
                {lvl1.title}
              </incmix.h4>
            )}
            ...
            {lvl1.routes.map((lvl2, index) => {
              ...
              const sortedRoutes = lvl2.sort ? sortRoutes(lvl2.routes) : lvl2.routes
                ...
                >
                  {sortedRoutes.map(lvl3 => (
                    <SidebarLink key={lvl3.path} href={lvl3.path}>
                      ...
                      {lvl3.new && (
                        <Badge ml="2" lineHeight="tall" fontSize="10px" variant="solid" colorScheme="purple">
                          New
                        </Badge>
                      )}
                      {lvl3.incmix && (
                        <Badge ml="2" lineHeight="tall" fontSize="10px" variant="solid" colorScheme="blue">
                          Incmix
                        </Badge>
                      )}
                    </SidebarLink>
...
export const mainNavLinks = [
  {
    ...
    match: (asPath: string, href: string) =>
      href.startsWith('/docs/styled-system') && asPath.startsWith('/docs/styled-system'),
  },
  {
    icon: <BsFillGridFill />,
  ...
  },
  {
    ...
    label: 'Hooks',
    match: (asPath: string, href: string) => href.startsWith('/docs/hooks') && asPath.startsWith('/docs/hooks'),
  },
]
