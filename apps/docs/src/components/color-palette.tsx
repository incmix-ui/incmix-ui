...
export const ColorPalettes = (props: { color: string }) => {
  ...
  return keys.map(item => <ColorPalette key={`${color}.${item}`} color={`${color}.${item}`} name={`${color} ${item}`} />)
}

export const ColorWrapper = (props: GridProps) => (
  <Grid mt={7} gap={6} templateColumns="repeat( auto-fit, minmax(200px, 1fr) )" {...props} />
)
